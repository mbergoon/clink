...
// Originally inspired by: http://golang.org/src/pkg/net/ipraw_test.go
...
func parseICMPMessage(b []byte) (*icmpMessage, error) {
	msglen := len(b)
	if msglen < 4 {
		return nil, errors.New("Error: Message is too short.")
	}
...
// Len represents the length of the icmpEcho accounting for seq/id fields.
...
func Echoer(address string, timeout int) (bool, int, error) {
	...
	wb, err := (&icmpMessage{
		...
		Body: &icmpEcho{
			ID: xid, Seq: xseq,
			Data: bytes.Repeat([]byte("Clink Clinkd Echo!!!"), 3),
		},
	}).Marshal()
	if err != nil {
